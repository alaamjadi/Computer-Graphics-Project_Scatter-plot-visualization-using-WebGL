let canvas, gl, shaderDir, baseDir;
...
// matrices
let modelMatrix, viewMatrix, projectionMatrix, matrix, normalMatrix;

// text elements to show on screen
let xAxisEl, yAxisEl, zAxis, textEl;

// grid vertex buffer and vertex count
let gridPositionBuffer, gridVertexCount;
...
function main() {
	utils.loadFiles([
	    ...
	    '/obj/cube.obj', '/obj/sphere.obj', 'dataset/data.json'
	], onFilesLoaded);
}

function onFilesLoaded(result) {
	// initialize the programs
	createPrograms(result[0], result[1],  result[2], result[3],  result[4], result[5]);

	// initial buffers for the object files
	parseObjs(result[6], result[7]);

	// form the normalized data
	parseJsonData(result[8]);
...
function initGridPositionBuffer() {
...
function createPrograms(main_vert, main_frag, grid_vert, grid_frag, pick_vert, pick_frag) {
	// create main program and get its attributes and uniforms
	program = utils.createAndCompileShaders(gl, [main_vert, main_frag]);
	...
	gridProgram = utils.createAndCompileShaders(gl, [grid_vert, grid_frag]);
	...
	pickProgram = utils.createAndCompileShaders(gl, [pick_vert, pick_frag]);
	pickAttributes = utils.fetchAttributes(pickProgram);
	pickUniforms = utils.fetchUniforms(pickProgram);
}

function parseObjs(cubeObjFile, sphereObjFile) {
	// parse cube.obj to get the vertex data
	let cubeData = utils.parseOBJ(cubeObjFile);
	...
	let sphereData = utils.parseOBJ(sphereObjFile);
